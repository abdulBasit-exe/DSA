...
public class Main {
    ...
    private final LList[] yearWiseLL=new LList[52];
    //a queue  for storing the biggest earth quack of each year
    private final LQueue earthQuackQueue=new LQueue();
    // linked list for storing the recent earth quack of each country with most recent on top
    private final LList recentCountryWiseQuacksList=new LList();
    //array of stacks for storing stack of every country
    private final LStack[] countryStack=new LStack[52];
    ...
    public Main(){
        ...
        for(int i=0;i<countryStack.length;i++)countryStack[i]=new LStack(totalDifferentCountries[i]);
        //Storing countries information in their corresponding stacks
        for (int i=0;i< country.length;i++)
            for (LStack myLinkedStack : countryStack)
    ...
    public void fetchingDataFromCSVFile(){
        ...
        File file=new File("src/DSA_PBl/Valid countries and cities.csv");
        try {
            int i=0;
...
/*
         ...
         */
        File f=new File("C:\\Users\\Abdul Basit\\IdeaProjects\\src\\DSA-JAVA\\src\\DSA_PBl/Country.txt");
        File f1=new File("C:\\Users\\Abdul Basit\\IdeaProjects\\src\\DSA-JAVA\\src\\DSA_PBl/Countries_Cities.txt");
        try {
    ...
    public void insertIntoQueue(){
        for (LList myLinkedList : yearWiseLL) earthQuackQueue.add(myLinkedList.getMax());
    }
    ...
    public LList[] getYearWiseLL() {
        return yearWiseLL;
    }

    public LQueue getEarthQuackQueue() {
        return earthQuackQueue;
    }

    public LList getRecentCountryWiseQuacksList() {
        return recentCountryWiseQuacksList;
    }

    public LStack[] getCountryStack() {
        return countryStack;
    }
